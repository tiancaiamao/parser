...
func (s *testParserSuite) TestDDL(c *C) {
	table := []testCase{
		...
		// for function
		{"CREATE FUNCTION say_hello(s text) RETURNS text DETERMINISTIC", true, "CREATE FUNCTION `say_hello`(`s` TEXT) RETURNS TEXT DETERMINISTIC SQL SECURITY DEFINER"},
		...
		{"CREATE FUNCTION sf1 (p1 BIGINT) RETURNS BIGINT   NOT DETERMINISTIC MODIFY SQL DATA BEGIN DECLARE ret INT DEFAULT 0; SELECT c1*2 INTO ret FROM t1 WHERE c1 = p1; RETURN ret; END", true, "CREATE FUNCTION `say_hello`((`s` TEXT) RETURNS TEXT DETERMINISTIC SQL SECURITY DEFINER"},
